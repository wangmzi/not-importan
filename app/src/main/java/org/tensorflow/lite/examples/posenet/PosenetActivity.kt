...
class PosenetActivity :
  ...
  private fun draw(canvas: Canvas, person: Person, personReal : Person, bitmap: Bitmap) {
    ...
    for (keyPoints in person.keyPoints) {
      ...
      if (keyPoints.score > minConfidence) {
        val position = keyPoints.position
    ...
    for(keyPoints in personReal.keyPoints) {
      if (keyPoints.score > minConfidence) {
        val position1 = keyPoints.position
        val realX: Float = position1.x.toFloat() * widthRatio + left
        val realY: Float = position1.y.toFloat() * heightRatio + top
        paint.color = Color.RED
        canvas.drawCircle(realX, realY, circleRadius, paint)
      }
    }
  ...
  private fun processImage(bitmap: Bitmap) {
    ...
    val drawedImage = ResourcesCompat.getDrawable(resources, R.drawable.yoga, null)
    val imageBitmap = drawableToBitmap(drawedImage!!)
    val scaledBitmapreal = Bitmap.createScaledBitmap(imageBitmap, MODEL_WIDTH, MODEL_HEIGHT, true)

    val person1 = posenet.estimateSinglePose(scaledBitmapreal)
    val canvas: Canvas = surfaceHolder!!.lockCanvas()
    draw(canvas, person, person1, scaledBitmap)
  }
